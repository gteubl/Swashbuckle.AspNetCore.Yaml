...
namespace Swashbuckle.AspNetCore.Swagger.Yaml
{
    public class SwaggerYamlMiddleware
    {
        ...
        private readonly ISerializer _swaggerSerializer;
        ...
        private async Task RespondWithSwaggerYaml(HttpResponse response, SwaggerDocument swagger)
        {
            ...
            using (var writer = new StringWriter(jsonBuilder))
            {
                ...
                rawJson = rawJson.Replace("unique-items", "uniqueItems");
                rawJson = rawJson.Replace("max-length", "maxLength");
                rawJson = rawJson.Replace("min-length", "minLength");
                rawJson = rawJson.Replace("read-only", "readOnly");
                rawJson = rawJson.Replace("\r\n\r\n", "\r\n");

                await response.WriteAsync(rawJson, new UTF8Encoding(false));
            }
        }
